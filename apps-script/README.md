...
- **public リポジトリでは `raw.githubusercontent.com` から取得します**（`GITHUB_TOKEN` 未設定時）。
  CDN配信なのでGitHub APIのレート制限を受けません。トークンを設定した場合のみ Contents API を使います。
- CSV内容のSHA-256を記録しており、**前回から変わっていなければ書き込みません**。
- 取得したCSVが空、またはヘッダしか無い場合は**中断します**（空データでの上書き防止）。
- 「認可一覧」の取得に失敗しても「定価一覧」の更新は続行します。
- 毎回タブ全体を書き換えるため、何度実行しても結果は同じです（冪等）。
- 5xx / 429 は指数バックオフで3回まで再試行します。

## うまくいかないとき

| 症状 | 原因と対処 |
| --- | --- |
| `ファイルが見つかりません (404)` | **まず疑うのはこれ**: Actions の update ジョブが未実行で `data/` が空。Actions タブから Run workflow する（push だけではテストしか走らない）。次に `GITHUB_REPO` / `GITHUB_REF` の綴り |
| `GitHubへのアクセスを拒否されました (403)` | public リポジトリなのに `GITHUB_TOKEN` が残っている（別リポジトリ用のトークンは403になる）。**プロパティごと削除する**。private の場合は権限と有効期限を確認 |
| `レート制限に達しました (403)` | トークン設定時のAPI制限。public なら `GITHUB_TOKEN` を消せばraw経由になり解消 |
| `CSVに行がありません` | Actions が動いたが解析結果が0件。Actions のログと `data/unparsed.json` を確認 |
| 実行時間が6分を超える | データが増えた場合。`CHUNK_ROWS` を小さくするか、Actions 側で列を絞る |
| 日本語が文字化けする | `Code.gs` を改変した場合のみ発生しうる。`getDataAsString('UTF-8')` を通していることを確認 |

> `GITHUB_TOKEN` は「空文字にする」のではなく**プロパティ行ごと削除**してください。
> 空文字なら未設定として扱われますが、削除のほうが確実です。
